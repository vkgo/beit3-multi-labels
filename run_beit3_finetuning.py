...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True, 
                        choices=['nlvr2', 'vqav2', 'flickr30k', 'coco_retrieval', 'coco_captioning', 'nocaps', 'imagenet', 'imagenetmultilabels'],
    ...
    parser.add_argument('--multilabels', action='store_true',
                        help='use multilabels in imagenet mode. Added by hwj.')
...
def main(args, ds_init):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.multilabels: # Multilabels Mode
                acc = test_stats['acc']
                pre = test_stats['pre']
                rec = test_stats['rec']
                f1 = test_stats['f1']
                threshold = test_stats['threshold']
                if best_metrics < f1:
                    best_metrics = f1
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch=f"f1{f1:.3f}_pre{pre:.3f}_rec{rec:.3f}_acc{acc:.3f}_threshold{threshold:.3f}", model_ema=model_ema)
                elif pre > 80:
                    if best_rec < rec:
                        best_rec = rec
                        if args.output_dir and args.save_ckpt:
                            utils.save_model(
                                ...
                                epoch=f"f1{f1:.3f}_pre{pre:.3f}_rec{rec:.3f}_acc{acc:.3f}_threshold{threshold:.3f}",
                                model_ema=model_ema)
            else:
                if best_metrics < test_stats[task_key]:
                    best_metrics = test_stats[task_key]
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch="best", model_ema=model_ema)

            print(f'Max performance: {best_metrics:.2f}')
            if log_writer is not None:
