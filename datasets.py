...
class ImageNetMultiLabelsDataset(BaseDataset):
    @staticmethod
    def get_index_files(split, task=None):
        if split == "train":
            return ("imagenet.train.index.jsonl",)
...
task2dataset = {
    ...
    "imagenetmultilabels": ImageNetMultiLabelsDataset,
}
...
def create_dataset_by_split(args, split, is_train=True):
    ...
    if args.task == 'imagenet' and args.multilabels:
        dataset_class = task2dataset['imagenetmultilabels']
    else:
