...
def multi_labels_metrics(output, target, mask):
    # Take only the relevant dimensions from the output tensor
    output = output[:, :target.shape[1]]

    metrics_by_threshold = {}

    for threshold in torch.arange(0.1, 0.91, 0.01):

        # Convert output tensor to binary values (0 or 1) using the current threshold
        output_binary = (output > threshold).float()

        # Calculate the element-wise equality between output_binary and target tensors
        correct = (output_binary == target).float()

        # Multiply the correct tensor by the mask tensor to only consider masked values
        masked_correct = correct * mask

        # Calculate the sum of correct predictions and the total number of masked elements
        num_correct = torch.sum(masked_correct).item()
        num_masked_elements = torch.sum(mask).item()

        # Calculate accuracy as the ratio of correct predictions to the total number of masked elements
        accuracy = num_correct / num_masked_elements if num_masked_elements > 0 else 0

        # Calculate recall for each sample
        true_positive = torch.sum(output_binary * target * mask)
        num_positive = torch.sum(target * mask)
        recall = true_positive / (num_positive + 1e-9)  # Adding a small constant to avoid division by zero

        # Calculate precision for each sample
        num_predicted_positive = torch.sum(output_binary * mask)
        precision = true_positive / (num_predicted_positive + 1e-9)  # Adding a small constant to avoid division by zero

        # Calculate F1 score for each sample
        f1 = 2 * (precision * recall) / (precision + recall + 1e-9)  # Adding a small constant to avoid division by zero

        # Store the metrics for the current threshold
        metrics_by_threshold[threshold.item()] = [accuracy * 100, recall * 100, precision * 100, f1]

    # Find the optimal metrics
    ...
    return threshold, optimal_metrics
...
def pad_classification_number(input_tensor: torch.Tensor, target_length: int = 1000) -> torch.Tensor:
    """
    将输入张量的classification_number维度补零至目标长度。

    :param input_tensor: 输入张量，形状为[batch, classification_number]
    :param target_length: 目标长度，默认为1000
    :return: 补零后的张量
    """
    assert len(input_tensor.shape) == 2, "输入张量的维度必须为2（[batch, classification_number]）。"

    current_length = input_tensor.shape[1]
    if current_length >= target_length:
        return input_tensor[:, :target_length]

    # 计算需要补零的数量
    pad_length = target_length - current_length

    # 使用nn.functional.pad来补零，需要一个4元素的元组，分别表示左、右、上、下的补零数
    padded_tensor = F.pad(input_tensor, (0, pad_length, 0, 0), mode='constant', value=0)
...
class ImageNetMultiLablesHandler(TaskHandler):
    def __init__(self, args) -> None:
        super().__init__()
        self.criterion = torch.nn.BCEWithLogitsLoss(reduction='sum')

    def train_batch(self, model, image, label, mask):
        logits = model(image=image)
        # Apply mask to logits and labels
        masked_logits = (logits * mask)
        masked_labels = (label * mask).float()

        # Calculate loss for selected logits and labels
        loss = self.criterion(masked_logits, masked_labels)
        loss = loss / torch.sum(masked_labels)
        # loss = torch.sum(loss)
    ...
    def eval_batch(self, model, image, label, mask):
        logits = model(image=image)
        batch_size = image.shape[0]
        # Apply mask to logits and labels
        best_threshold, metrics = multi_labels_metrics(logits, label, mask)
        self.metric_logger.meters['threshold'].update(best_threshold, n=batch_size)
        self.metric_logger.meters['acc'].update(metrics[0], n=batch_size)
        self.metric_logger.meters['rec'].update(metrics[1], n=batch_size)
        self.metric_logger.meters['pre'].update(metrics[2], n=batch_size)
        self.metric_logger.meters['f1'].update(metrics[3], n=batch_size)

    def after_eval(self, **kwargs):
        print('* Threshold {threshold.global_avg:.3f} *\n'
              '* Acc {acc.global_avg:.3f}% *\n'
              '* Rec {rec.global_avg:.3f}% *\n'
              '* Pre {pre.global_avg:.3f}% *\n'
              '* F1 {f1.global_avg} *'
              .format(acc=self.metric_logger.acc, rec=self.metric_logger.rec,
                      pre=self.metric_logger.pre, f1=self.metric_logger.f1,
                      threshold=self.metric_logger.threshold))
        return {k: meter.global_avg for k, meter in self.metric_logger.meters.items()}, "acc"
...
def get_handler(args):
    if args.task == "nlvr2":
        ...
        if args.multilabels:
            return ImageNetMultiLablesHandler(args)
        else:
...
def train_one_epoch(
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if multilabels:
            # do padding on tensor
            data['label'], data['mask'] = pad_classification_number(data['label']), pad_classification_number(data['mask'])
        else:
